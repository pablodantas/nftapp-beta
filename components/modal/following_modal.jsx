...
function Following({ keyModal, showElementFOLLOWING, seguindo, closeButton  }) {
  const ref = useRef();
  ...
  const { data, isLoading, error } = useQuery(
    ...
    {
      staleTime: 1000 * 90,
      refetchInterval: 5000, // a consulta será reexecutada a cada 5 segundos
      cacheTime: 1000 * 60 * 5,
    }
  );
  ...
  async function handleSeguir(follow, key) {
    ...
    try {
      if (b && a && a !== b) {
        ...
        query.equalTo("seguindo", a);
        query.equalTo("seguidores", b);
        ...
        <div className={"modal fade show block"} key={keyModal}>
          <div className="modal-dialog w-40vw">
            <div className="modal-content dark:bg-jacarta-700" ref={ref}>
              ...
              <div className="modal-body p-over_20 h-70">
                {!isLoading && item.length === 0 && (
                  <p className="text-center p-3">
                    You are not following anyone to display.
                  </p>
                )}
              ...
              {isLoading ? (
                <div className="loader-container flex justify-center btn_disable mb-2">
                  <div className="loader"></div>
                </div>
              ) : (
                  <button
